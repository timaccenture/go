...
func playGame(m *nats.Msg) string {
	// m.Subject sollte rps mit der gameid sein
	// m.Data sollte den spielzug enthalten

	validInputs := []string{"rock", "paper", "scissor"}
	input := string(m.Data)

	if !slices.Contains(validInputs, input) {
		return "pls provide valid input (rock, paper or scissor)"
	}
	//let computer play
	...
	if diff == 0 {
		return "You've tied. The other player chose " + compChoice + " too."
	} else if slices.Contains(userMinusComp["User won"], diff) {
		return "Congrats, you won! The other player chose " + compChoice
	} else if slices.Contains(userMinusComp["Comp won"], diff) {
		return "You've lost :( The other player chose " + compChoice
	} else {
		return "Ooops, sth went wrong!"
	}
}

func showLobby(m *nats.Msg, i int) string {
	// implement lobby that shows
	return ""
}
